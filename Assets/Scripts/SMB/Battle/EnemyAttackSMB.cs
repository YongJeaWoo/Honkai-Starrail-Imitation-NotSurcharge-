...
public class EnemyAttackSMB : StateMachineBehaviour
{
    ...
    override public void OnStateExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        {
            if (fsmController is BattleEnemyFSMController)
            {
                ((BattleEnemyFSMController)fsmController).TransactionToState(E_BattleEnemyState.Idle);
            }
            else if (fsmController is BattleBossFSMController)
            {
